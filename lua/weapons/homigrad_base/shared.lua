...
if CLIENT then
	...
	local ammoInspectDoubleTapWindow = 0.32
	local ammoInspectDuration = 2.75
	...
	function SWEP:HandleAmmoInspectDoubleTap()
		local owner = self:GetOwner()
		if not IsValid(owner) or owner ~= LocalPlayer() then return end
		if not owner:Alive() or not owner:KeyPressed(IN_RELOAD) then return end
		if self:GetPrimaryAmmoType() < 0 or self:GetMaxClip1() <= 0 or self:Clip1() < 0 then return end

		local time = CurTime()
		self.lastAmmoInspectReloadTap = self.lastAmmoInspectReloadTap or 0

		if time - self.lastAmmoInspectReloadTap <= ammoInspectDoubleTapWindow then
			self.hudinspect = time + ammoInspectDuration
			self.lastAmmoInspectReloadTap = 0
			return
		end

		self.lastAmmoInspectReloadTap = time
	end

	function SWEP:DrawHUD()
		if not IsValid(self:GetOwner()) then return end
		self:HandleAmmoInspectDoubleTap()
