...
function SWEP:Attack(owner, ent, vellen, attacktype, inattackLength)
    ...
    for i = 0, amt do
        ...
        local traceReach = (attacktype and 1 or math.max(0.5, 1 - math.abs((0.5 - inattackLength) * 2))) * (self:GetAttackLength() + vellen)
        tr.endpos = eyetr.StartPos + normal:Forward() * (traceReach + math.max((self.AttackSize or 5) * 0.2, 1))
        tr.filter = self.MultiDmg1 and {owner, ent} or self.HitEnts

        local size = math.max(1.1, (self.AttackSize or 5) * 0.28) * (attacktype and 0.9 or 1)
...
function SWEP:PlayEffects(trace, attacktype)
    ...
    if self:IsEntSoft(trace.Entity) then
        ...
        if CLIENT and owner == LocalPlayer() and IsFirstTimePredicted() then
            ViewPunch(Angle(attacktype and -0.5 or -0.8, attacktype and 0.2 or 0.35, 0))
        end
