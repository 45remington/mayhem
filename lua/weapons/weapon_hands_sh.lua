...
SWEP.DamageMul = 1
SWEP.animtime = 0
SWEP.PunchDeployDelay = 0.16
SWEP.PunchCooldown = 0.22
SWEP.PunchSpecialCooldown = 0.55
...
local function PickPunchTrace(bestCreature, bestObj, bestOther)
	if bestCreature then return bestCreature.Entity, bestCreature.HitPos, bestCreature.HitNormal, bestCreature.PhysicsBone, bestCreature end
	if bestObj then return bestObj.Entity, bestObj.HitPos, bestObj.HitNormal, bestObj.PhysicsBone, bestObj end
	if bestOther then return bestOther.Entity, bestOther.HitPos, bestOther.HitNormal, bestOther.PhysicsBone, bestOther end
	return
end

local function WhomILookinAt(ply, cone, dist)
	local CreatureTr, ObjTr, OtherTr
	local start = hg.eye(ply)
	local aim = ply:GetAimVector()
	local ang = ply:EyeAngles()
	local right = ang:Right()
	local up = ang:Up()
	local filter = {ply, hg.GetCurrentCharacter(ply)}
	local sweepMul = math.max(cone or 0.3, 0.2) * 0.12

	local function storeTrace(tr)
		if not tr or not tr.Hit or tr.HitSky or not IsValid(tr.Entity) then return end

		local ent = tr.Entity
		local class = ent:GetClass()
		if ent:IsPlayer() or ent:IsNPC() then
			CreatureTr = CreatureTr or tr
		elseif class == "prop_physics" or class == "prop_physics_multiplayer" or class == "prop_ragdoll" then
			ObjTr = ObjTr or tr
		else
			OtherTr = OtherTr or tr
		end
	end

	for _, offset in ipairs(punchSweepOffsets) do
		local dir = (aim + right * (offset.x * sweepMul) + up * (offset.y * sweepMul)):GetNormalized()
		storeTrace(util.TraceHull({
			start = start,
	...
	if CreatureTr or ObjTr or OtherTr then
		return PickPunchTrace(CreatureTr, ObjTr, OtherTr)
	end

	local fallback = util.TraceHull({
		start = start,
		endpos = start + aim * dist,
		filter = filter,
		mins = Vector(-9, -9, -9),
		maxs = Vector(9, 9, 9),
		mask = MASK_SHOT_HULL
...
local trMins, trMaxs = Vector(-7, -7, -7), Vector(7, 7, 7)
local trMinsClaws, trMaxsClaws = Vector(-9, -9, -9), Vector(9, 9, 9)
...
function SWEP:PrimaryAttack(forcespecial)
	...
	if not self:GetFists() then
		self:SetFists(true)
		self:DoBFSAnimation("fists_draw",1)
		self:SetNextPrimaryFire(CurTime() + self.PunchDeployDelay)
		return
	end
	...
	self.attacked = CurTime() + 0.1

	local special_attack = owner:KeyDown(IN_USE)
	...
	local punchCooldown = (special_attack and self.PunchSpecialCooldown or self.PunchCooldown) * math.Clamp((180 - owner.organism.stamina[1]) / 90, 1, 2)
	punchCooldown = punchCooldown + (math.max(special_attack and 0.45 or 0, clawClasses[owner.PlayerClassName] or 0))
	self:SetNextPrimaryFire(CurTime() + punchCooldown)
	self:SetNextSecondaryFire(CurTime() + punchCooldown * 0.95)
...
function SWEP:AttackFront(special_attack, rand)
	...
	local reachDistance = self.ReachDistance + (special_attack and 4 or 0)
	local Ent, HitPos, _, physbone, trace = WhomILookinAt(owner, special_attack and 0.45 or 0.32, reachDistance)
	if clawClasses[owner.PlayerClassName] then
		local pos = hg.eye(owner)
		trace = util.TraceHull({
			start = pos,
			endpos = pos + owner:GetAimVector() * reachDistance,
	...
	if IsValid(Ent) or (Ent and Ent.IsWorld and Ent:IsWorld()) then
		...
		owner:ViewPunch(Angle(special_attack and -4 or -2, rand and -1 or 1, rand and 1.5 or -1.5))
		util.ScreenShake(HitPos, special_attack and 8 or 5, 55, 0.1, 90, false)

		if Ent:IsPlayer() then
			Ent:ViewPunch(Angle(special_attack and -55 or -9, rand and 2 or -2, rand and 1 or -1))
		end

		if IsValid(Phys) then
			if Ent:IsPlayer() then
				Ent:SetVelocity(AimVec * SelfForce * (special_attack and 2 or 1.7) * (owner.organism.superfighter and 2 or 1) * (isZomb and 4 or 1) * (1 + owner.organism.berserk * 5))
			end
			Phys:ApplyForceOffset(AimVec * (special_attack and 6200 or 5400) * Mul * (isZomb and 3 or 1), HitPos)
			owner:SetVelocity(AimVec * SelfForce * (special_attack and 0.9 or 0.7) * (owner.organism.superfighter and 2 or 1) * (isZomb and 2 or 1) * (1 + owner.organism.berserk / 10))
		end
	end
