...
function SWEP:PrimaryAttack(forcespecial)
	...
	if not self:GetFists() then
		if IsValid(self.CarryEnt) or IsValid(owner:GetNetVar("carryent")) or IsValid(owner:GetNetVar("carryent2")) then
			return
		end
	...
	self:SetNextPrimaryFire(CurTime() + .45 * math_Clamp((180 - owner.organism.stamina[1]) / 90,1,2) + (special_attack and 0.5 or isfur and 0.4 or 0))
	self:SetNextSecondaryFire(CurTime() + .45 + (special_attack and 0.5 or isfur and 0.4 or 0))
