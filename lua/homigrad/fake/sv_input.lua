...
hook.Add("OnPlayerHitGround","fallStun",function(ply,inwater,onfloater,speed)
	...
	if org and speed > 500 then
		local leg = math.random(2) == 1 and "rleg" or "lleg"
		local name = leg == "rleg" and "right leg" or "left leg"
		local broken = speed > 750
		if broken then
			org[leg] = 1
		else
			org[leg .. "dislocation"] = true
		end
		ply:Notify("Your " .. name .. " was " .. (broken and "broken" or "dislocated") .. ".", 0, "fall_leg_" .. tostring(CurTime()), 0, nil, Color(255, 80, 80))
	end
end)
