...
local shotColor = Color(255, 80, 80)
local function GetShotArea(boneName)
	boneName = string.lower(boneName or "")
	if string.find(boneName, "head") or string.find(boneName, "neck") then return "head" end
	if string.find(boneName, "spine") or string.find(boneName, "chest") then return "torso" end
	if string.find(boneName, "pelvis") then return "pelvis" end
	if string.find(boneName, "r_.*arm") or string.find(boneName, "r_.*hand") then return "right arm" end
	if string.find(boneName, "l_.*arm") or string.find(boneName, "l_.*hand") then return "left arm" end
	if string.find(boneName, "r_.*thigh") or string.find(boneName, "r_.*calf") or string.find(boneName, "r_.*foot") then return "right leg" end
	if string.find(boneName, "l_.*thigh") or string.find(boneName, "l_.*calf") or string.find(boneName, "l_.*foot") then return "left leg" end
	return "body"
end
...
function hg.organism.AddWound(ent, tr, bone, dmgInfo, dmgPos, dmgBlood, inputHole, outputHole)
	...
	if dmgInfo:IsDamageType(DMG_BULLET + DMG_BUCKSHOT) and IsValid(org.owner) and org.owner:IsPlayer() and (org.owner.NextShotNotify or 0) < CurTime() then
		org.owner.NextShotNotify = CurTime() + 0.35
		org.owner:Notify("You were shot in the " .. GetShotArea(ent:GetBoneName(bone)) .. ".", 0, "shot_" .. tostring(CurTime()), 0, nil, shotColor)
	end
