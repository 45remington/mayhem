...
local injuryColor = Color(255, 80, 80)
local limbNames = {
	rarm = "right arm",
	larm = "left arm",
	rleg = "right leg",
...
local function NotifyLimbInjury(org, key, injury, dmgInfo)
	if !org.isPly or org[key.."amputated"] then return end
	local cause = dmgInfo:IsDamageType(DMG_FALL) and " from the fall" or dmgInfo:IsDamageType(DMG_BULLET + DMG_BUCKSHOT) and " from the shot" or dmgInfo:IsDamageType(DMG_CRUSH) and " from the impact" or ""
	org.owner:Notify("Your " .. limbNames[key] .. " was " .. injury .. cause .. ".", 0, injury..key..tostring(CurTime()), 0, nil, injuryColor)
end
...
local function legs(org, bone, dmg, dmgInfo, key, boneindex, dir, hit, ricochet)
	...
	if dmg >= 1 and (!dmgInfo:IsDamageType(DMG_CLUB+DMG_CRUSH+DMG_FALL) or math.random(3) != 1) then
		...
		NotifyLimbInjury(org, key, "broken", dmgInfo)
		...
		NotifyLimbInjury(org, key, "dislocated", dmgInfo)
