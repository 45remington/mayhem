...
concommand.Add("hg_organism_clear", function(ply, cmd, args)
	...
	if not args[1] then
		...
		ply:ScreenMessage("YOU HAVE BEEN GIVEN A SECOND CHANCE BY THE UPPERS", "secondchance")
	end

	if args[1] then
		for i,pl in pairs(player.GetListByName(args[1])) do
			hg.organism.Clear(pl.organism)
			pl:ScreenMessage("YOU HAVE BEEN GIVEN A SECOND CHANCE BY THE UPPERS", "secondchance")
		end
	end
end)
