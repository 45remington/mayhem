...
local function runDropWeapon(class)
	if class then
		RunConsoleCommand("drop", class)
	else
		RunConsoleCommand("drop")
	end
end

local function getDroppableWeaponOptions()
	local options = {}
	local used = {}

	local function addWeapon(class, wep)
		if not class or class == "weapon_hands_sh" or used[class] then return end
		if IsValid(wep) and wep.NoDrop then return end
		if not IsValid(wep) and not lply:HasWeapon(class) then return end

		local data = weapons.Get(class)
		options[#options + 1] = {class, data and data.PrintName or class}
		used[class] = true
	end

	local activeWeapon = lply:GetActiveWeapon()
	if IsValid(activeWeapon) then addWeapon(activeWeapon:GetClass(), activeWeapon) end

	local inventory = lply:GetNetVar("Inventory", {})
	local inventoryWeapons = inventory.Weapons or {}
	for class in SortedPairs(inventoryWeapons) do
		addWeapon(class, lply:GetWeapon(class))
	end

	return options
end

local function getActiveDroppableWeaponClass()
...
local function dropWeapon(mouseClick)
	local activeClass = getActiveDroppableWeaponClass()
	local options = getDroppableWeaponOptions()

	if activeClass and (mouseClick == 1 or #options <= 1) then
		runDropWeapon(activeClass)
		return
	end

	if #options == 1 then
		runDropWeapon(options[1][1])
		return
	end

	local menuOptions = {}
	for _, option in ipairs(options) do
		local class = option[1]
		menuOptions[#menuOptions + 1] = {function() runDropWeapon(class) end, option[2]}
	end

	hg.CreateRadialMenu(menuOptions)
end

hook.Add("radialOptions", "77", function()
	local organism = lply.organism or {}
	if not organism.otrub and #getDroppableWeaponOptions() > 0 then
